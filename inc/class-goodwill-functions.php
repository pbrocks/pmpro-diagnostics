...
class Goodwill_Functions {
	...
	function __construct() {
		...
		// add_action( 'wp_enqueue_scripts', array( $this, 'localize_deliver_promise' ) );
		// add_action( 'wp_head', array( $this, 'deliver_promise_loaded' ), 55 );
		// add_action( 'init', array( $this, 'send_mail_loaded' ), 55 );
	}

	public function send_mail_loaded() {
	...
	public function deliver_promise_loaded() {
		echo '<h4 style="text-align:center;color:salmon;">' . __FUNCTION__ . '</h4>';
		$handle = 'deliver';
		$list = 'registered';
		if ( wp_script_is( $handle, $list ) ) {
			// echo '<h4 style="text-align:center;color:salmon;">' . plugins_url( '/js/deliver-promise.js', __FILE__ ) . '</h4>';
		}
		if ( class_exists( 'Goodwill_User' ) ) {
			echo '<h4 style="text-align:center;color:salmon;">' . plugins_url( '/goodwill-user', __FILE__ ) . '</h4>';
		}
	}

	public function localize_deliver_promise() {
		wp_register_script(
			'deliver',
			plugins_url( '/js/deliver-promise.js', __FILE__ ),
			array( 'jquery', 'bootstrap', 'goodwill' ),
			'1.0'
		);
		wp_enqueue_script( 'deliver' );
		// wp_localize_script( 'deliver', 'email_template_data', $email_template_data );
	}
	...
	function goodwill_end_session() {
		$goodwill_user = Goodwill_User::get_instance();
	...
	function goodwill_fetch_contacts() {
		...
		$gwUser = Goodwill_User::get_instance();
	...
	function goodwill_fetch_notes() {
		$gw_user = GoodWill_User::get_instance();
	...
	function goodwill_get_email_templates() {
		...
		foreach ( $raw_email_templates as $raw_email_template ) {
			$template = new Goodwill_Email_Template();
